...
export default function PlayerProfile({ isOpen, onClose, playerName }: PlayerProfileProps) {
	...
	useEffect(() => {
		const loadPlayerData = () => {
			...
			const savedAchievements = localStorage.getItem(`deep-cut-achievements-${playerName}`);
			...
			if (savedStats) {
				...
				const demoStats: PlayerStats = {
					...
					favoriteThemes: ["Childhood Secrets", "Work Confessions", "Digital Shame"],
				...
				setStats(demoStats);
				localStorage.setItem(`deep-cut-stats-${playerName}`, JSON.stringify(demoStats));
			}
			...
			if (savedAchievements) {
				...
				localStorage.setItem(`deep-cut-achievements-${playerName}`, JSON.stringify([...demoUnlocked]));
			}
		};
	...
	const averageGameTime = stats.gamesPlayed > 0 ? Math.round(stats.playTime / stats.gamesPlayed) : 0;
						...
						>
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
								<div className="bg-zinc-800/50 rounded-lg p-4 text-center">
									<div className="text-3xl font-bold text-pink-500">{stats.gamesPlayed}</div>
								...
								<div className="bg-zinc-800/50 rounded-lg p-4 text-center">
									<div className="text-3xl font-bold text-green-500">{stats.totalSubmissions}</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-4">
									<h3 className="text-lg font-semibold text-pink-400">Performance</h3>
									<div className="space-y-3">
										<div className="flex justify-between">
											<span className="text-zinc-400">Average Votes per Submission</span>
											<span className="text-white">{stats.averageVotes.toFixed(1)}</span>
										</div>
										<div className="flex justify-between">
								...
								<div className="space-y-4">
									<h3 className="text-lg font-semibold text-pink-400">Activity</h3>
									<div className="space-y-3">
										<div className="flex justify-between">
											<span className="text-zinc-400">Total Play Time</span>
											<span className="text-white">{Math.floor(stats.playTime / 60)}h {stats.playTime % 60}m</span>
										</div>
										<div className="flex justify-between">
							...
							{/* Favorite Themes */}
							<div>
								<h3 className="text-lg font-semibold text-pink-400 mb-4">Favorite Themes</h3>
						...
						>
							...
							<div className="bg-zinc-800/30 rounded-lg p-4">
								<div className="flex justify-between items-center mb-2">
									...
									<span className="text-white">{unlockedAchievements.size}/{allAchievements.length}</span>
								</div>
								<div className="w-full bg-zinc-700 rounded-full h-2">
									<div
										className="bg-gradient-to-r from-pink-500 to-purple-600 h-2 rounded-full transition-all duration-500"
										style={{ width: `${(unlockedAchievements.size / allAchievements.length) * 100}%` }}
									/>
								</div>
							</div>
									...
									return (
										<motion.div
											...
											className={`rounded-lg p-4 border-2 transition-all ${
												isUnlocked
													? `${getRarityColor(achievement.rarity)} bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 shadow-lg ${getRarityGlow(achievement.rarity)}`
										...
										>
											<div className="flex items-center gap-3 mb-2">
												...
												<div>
													<h3 className={`font-semibold ${isUnlocked ? "text-white" : "text-zinc-500"}`}>
														{achievement.name}
													</h3>
													<div className={`text-xs capitalize ${getRarityColor(achievement.rarity).split(' ')[0]}`}>
														{achievement.rarity}
													</div>
												</div>
											</div>
					...
					{activeTab === "settings" && (
						...
						>
							<div className="text-center py-12">
								...
								<h3 className="text-xl font-semibold text-zinc-400 mb-2">Player Settings</h3>
								<p className="text-zinc-500 mb-6">Customize your Deep Cut experience</p>
								...
								<div className="space-y-4 max-w-md mx-auto">
									<Button
										...
										onClick={() => {
											...
											localStorage.removeItem(`deep-cut-achievements-${playerName}`);
									...
									<Button
										...
										onClick={() => {
											const data = {
												stats: localStorage.getItem(`deep-cut-stats-${playerName}`),
												achievements: localStorage.getItem(`deep-cut-achievements-${playerName}`)
											};
